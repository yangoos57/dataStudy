...
- **Serverless니까 Aurora 처럼 Read Replica 같은걸 설정할 필요가 없음**
...
- Kinesis Data Streams을 Kinesis Data Firehose로 교체한다.(Incorrect)
- Kinesis Data Streams을 SQS FIFO 대기열로 교체한다.
- Kinesis Data Streams을 SQS 표준 대기열로 교체한다.
- **Kinesis Data Streams의 향상된 팬아웃(Enhanced Fanout) 기능을 활성화한다.(Correct)**

정답

- IOT와 Firehose는 연동되지 않는다.
- Enhanced Fanout을 쓰면 Data Stream 성능을 향상시킬 수 있다.
...
- 30일 뒤에 객체를 Amazon S3 Standard-Infrequent Access(S3 Standard-IA)로 전환하도록 수명 주기 정책을 설정한다.
- 7일 뒤에 객체를 Amazon S3 One Zone-Infrequent Access(S3 One Zone-IA)로 전환하도록 수명 주기 정책을 설정한다.
- **30일 뒤에 객체를 Amazon S3 One Zone-Infrequent Access(S3 One Zone-IA)로 전환하도록 수명 주기 정책을 설정한다.(Correct)**
- 7일 뒤에 객체를 Amazon S3 Standard-Infrequent Access(S3 Standard-IA)로 전환하도록 수명 주기 정책을 설정한다.(Incorrect)

****Explanation****

- 최소 저장 기간이 있음. Onezone-IA의 경우 30일이다.
...
- Spread 그룹에서 단일 AZ 내 최대 가용은 7개이다.
- 7*3 = 21이므로 3개가 필요함.
...
- **EBS 볼륨이 EFS 파일 시스템에 백업되지 않아 볼륨이 손실되었다.**
- **EC2 인스턴스가 종료되면 연결된 모든 EBS 볼륨이 항상 종료된다.**
- **EBS 볼륨이 Amazon S3 스토리지에 백업되지 않아 볼륨이 손실되었다.**
- **EBS 볼륨이 Amazon EC2 인스턴스의 루트 볼륨으로 설정되었다. 인스턴스를 종료하면 기본 동작에 의해 연결된 루트 볼륨도 함께 종료된다.(Correct)**
...
- **EC2 온디맨드 인스턴스를 사용한다.**
- **EC2 예약 인스턴스를 사용한다.**
- **EC2 전용 인스턴스를 사용한다.(Incorrect)**
- **EC2 전용 호스트를 사용한다.(Correct)**

### **Explanation**

- 라이선스 사용 = 전용 호스트 사용
- 전용 인스턴스는 기존의 서버와 연계된 소프트웨어 라이선스에 사용할 수 없습니다.
...
- **CloudWatch 경보(Alarm)를 구성해 인스턴스의 상태를 모니터링한다. 인스턴스 상태 확인이 실패하면 CloudWatch 경보가 SNS 이벤트를 발행해 Lambda 함수를 호출할 수 있다. Lambda 함수는 AWS EC2 API를 사용해 인스턴스를 재부팅할 수 있다.(Incorrect)**
- **CloudWatch 경보(Alarm)를 구성해 인스턴스의 상태를 모니터링한다. 인스턴스 상태 확인이 실패하면 CloudWatch 경보 액션을 사용해 인스턴스를 재부팅할 수 있다.(Correct)**
...
![Untitled](UDemy%20AWS%202b59be6a38524858b63a4b507501541a/Untitled%20313.png)
...
- **Amazon S3에서 버킷 액션을 대한 EventBridge 이벤트를 구성한다. EventBridge 이벤트에서 AWS Lambda 함수가 호출되면 Amazon Kinesis Data Streams으로 필요한 데이터를 전송한다.**
- **Amazon S3 버킷 액션을 직접 구성하여 Amazon Simple Notification Service(SNS)에 데이터를 기록한다. 그런 다음 SNS를 사용해 업데이트 내용을 Amazon Kinesis Data Streams로 전송한다.**
- **AWS 데이터베이스 마이그레이션 서비스(AWS DMS)를 Amazon S3와 Amazon Kinesis Data Streams 간의 브리지로 활용한다.(Correct)**
- **S3 이벤트 알림을 활용해 파일 생성 이벤트를 위한 Lambda 함수를 호출한다. 호출된 Lambda 함수가 Amazon Kinesis Data Streams로 필요한 데이터를 전송한다.(Incorrect)**

****Explanation****

- On-Premise로 데이터를 옮기는 방법 ⇒ DMS 사용
...
- [ ]  **둘 이상의 Lambda 함수에서 코드를 재사용할 거라면 코드 재사용이 가능한 Lambda 계층 생성을 고려해야 한다.(Correct)**
- [x]  **기본적으로 Lambda 함수는 항상 AWS 소유의 VPC에서 동작하기 때문에 공용 인터넷 주소나 공용 AWS API에 접근할 수 있다. Lambda 함수가 VPC를 사용하도록 설정되어 있다면 퍼블릭 서브넷에서 NAT 게이트웨이를 통해 라우팅하여 퍼블릭 리소스에 접근해야 한다.(Correct)**
- [x]  **서버리스 아키텍처와 컨테이너는 서로를 보완하지만 Lambda 함수를 컨테이너 이미지로 패키징하여 배포할 수는 없다.(Incorrect)**
- [x]  **Lambda 함수는 아주 빠르게 늘어날 수 있기 때문에 동시 실행(Concurrent Executions) 또는 동시 호출과 같은 함수 지표가 예상 임계값을 초과했을 때 팀에게 알려줄 수 있도록 CloudWatch 경보를 배포하는 것이 좋다.(Correct)**
...
- **AWS CodeDeploy를 사용해 배포 옵션을 적절한 배포 옵션으로 지정한다.**
- **AWS Global Accelerator를 사용해 트래픽의 일부를 특정 배포로 분산한다.(Correct)**
- **일랙스틱 로드 밸런서를 사용해 트래픽을 여러 배포로 분산한다.**
- **AWS Route 53 가중치 라우팅을 사용해 트래픽을 서로 다른 배포로 분산한다.(Incorrect)**

****Explanation****

AWS 글로벌 액셀러레이터는 하나 또는 다수의 AWS 리전에서 애플리케이션 로드 밸런서, 네트워크 로드 밸런서, 탄력적 IP 주소 또는 Amazon EC2 인스턴스 등 여러분의 애플리케이션 엔드포인트에 대한 고정된 진입점 역할을 하는 정적 애니캐스트(anycast) IP 주소를 2개 제공합니다.
...
- [x]  **S3 콘솔을 사용해 S3 전송 가속화(Transfer Acceleration)를 활성화하여 다른 리전의 S3 버킷으로 객체를 복사한다.(Incorrect)**
- [ ]  **S3 콘솔을 사용해 소스 S3 버킷에서 대상 S3 버킷으로 데이터를 복사한다.**
- [x]  **aws S3 sync 명령어를 사용해 소스 버킷에서 대상 버킷으로 데이터를 복사한다.(Correct)**
- [ ]  **Snowball Edge 장치를 사용하여 한 리전에서 다른 리전으로 데이터를 복사한다.**
- [x]  **S3 콘솔을 사용해 S3 배치 복제(Batch Replication)를 구성하여 다른 리전의 S3 버킷으로 객체를 복사한다.(Correct)**

****Explanation****

- Region 간 복사를 위한 방법
    - aws S3 Sync를 사용한다.
    - S3 Batch Replication을 사용한다.
...
- [ ]  **인스턴스가 실행 중일 때, 루트 사용자 자격 증명을 사용해 사용자 데이터를 업데이트할 수 있다.**
- [ ]  **사용자 데이터는 기본적으로 EC2 인스턴스가 다시 시작될 때마다 실행된다.**
- [ ]  **사용자 데이터로 입력된 스크립트에는 기본적으로 실행에 대한 루트 사용자 권한이 없다.**
- [x]  **사용자 데이터는 기본적으로 인스턴스를 처음 시작할 때의 부팅 주기 동안만 실행된다.(Correct)**
- [x]  **사용자 데이터로 입력된 스크립트는 기본적으로 루트 사용자 권한으로 실행된다.(Correct)**

****Explanation****

- User 데이터는 한 번만 실행되고 root user 권한으로 실행된다.
...
- [x]  **AWS Snowball Edge(Incorrect)**
- [x]  **AWS Database Migration Service(Correct)**
- [ ]  **Basic Schema Copy**
- [x]  **AWS Schema Converion Tool(Correct)**
- [ ]  **AWS Glue**

****Explanation****

- 시험에 Snowball 장치가 나올 수 있지만 원래의 Snowball 장치는 저장 공간이 80TB라는 것만 기억하시면 됩니다. **AWS Snowball Edge는 데이터베이스 마이그레이션에 사용할 수 없습니다.**
- **데이터베이스 마이그레이션 사용 불가(데이터 마이그레이션 전용)**

![Untitled](UDemy%20AWS%202b59be6a38524858b63a4b507501541a/Untitled%20314.png)

q31

- **S3에서 Cross-Account를 사용하려면, IAM Policy를 설정해야한다.**

q37

**다음 중 Amazon VPC 콘솔 마법사에서 지원하지 않는 네트워크 구성은 무엇입니까?**

- **하나의 퍼블릭 서브넷을 가진 VPC**
- **퍼블릭 및 프라이빗 서브넷이 있는 VPC와 AWS Site-to-Site VPN 액세스**
- **퍼블릭 서브넷만 있는 VPC와 AWS Site-to-Site VPN 액세스(Correct)**
- **퍼블릭 및 프라이빗 서브넷이 있는 VPC(NAT)(Incorrect)**
...
- NLB는 Private IP를 통해 Target에 트래픽을 제공한다.

q41

VIF = Virtural InterPface / 
...
- [ ]  **AWS 클라우드와 유럽 및 아시아 지사 간에 Site-to-Site VPN 연결을 여러 개 생성하고, VPN 연결을 통해 S3로 더 빠르게 파일을 업로드한다.**
- [x]  **멀티파트(multipart) 업로드를 사용해 목적지 S3 버킷으로 더 빠르게 파일을 업로드한다.(Correct)**
- [x]  **AWS Global Accelerator를 사용해 목적지 S3 버킷으로 더 빠르게 파일을 업로드한다.(Incorrect)**
- [ ]  **S3 전송 가속화(Transfer Acceleration)를 사용하여 목적지 S3 버킷으로 더 빠르게 파일을 업로드한다.(Correct)**
- [ ]  **AWS 클라우드와 유럽 및 아시아 지사 간에 AWS Direct Connect 연결을 여러 개 생성하고, Direct Connect 연결을 통해 S3로 더 빠르게 파일을 업로드한다.**

****Explanation****

- 문제가 업로드 속도를 올리는 것이므로 Global Accelerator 보단 Transfer Acceleration이 효과적이다.
- Amazon S3 Transfer Acceleration을 사용하면 여러분의 클라이언트와 S3 버킷 사이의 먼 거리에 걸쳐 파일들을 빠르고 쉬우며 안전하게 전송할 수 있습니다. Transfer Acceleration은 Amazon CloudFront의 전 세계적으로 분포된 엣지 위치를 활용합니다. 데이터가 엣지 위치에 도달하면 최적화된 네트워크 경로를 거쳐 데이터가 Amazon S3로 라우팅됩니다.
    ...
    ![Untitled](UDemy%20AWS%202b59be6a38524858b63a4b507501541a/Untitled%20315.png)
    

q43

- **Data Migration Service는 On-premise 뿐만 아니라 AWS 서비스간 데이터베이스 이동에도 효과적이다.**
- **Source와 Target의 차이가 느껴지는데, Target을 보면 Redshift, DynamoDB, Kafka, Kinesis 등 Source에 없는 서비스가 보인다**

![Untitled](UDemy%20AWS%202b59be6a38524858b63a4b507501541a/Untitled%20316.png)
...
- **Lambda 함수에 S3 버킷에 대한 액세스 권한을 부여하는 IAM 역할을 생성한다. 해당 IAM 역할을 Lambda 함수의 실행 역할로 설정하면 Lambda 함수에 S3 버킷에 대한 교차 계정 액세스 권한이 부여된다.**
- **S3 버킷 소유자가 다른 AWS 계정에 있는 Lambda 함수가 액세스할 수 있도록 버킷을 퍼블릭으로 설정해야 한다.(Incorrect)**
- **AWS Lambda는 다른 AWS 계정에 있는 리소스에 액세스할 수 없다. 아이덴티티 페더레이션을 사용해 Lambda의 제약 사항을 우회해야 한다.**
- **Lambda 함수에 S3 버킷에 대한 액세스 권한을 부여하는 IAM 역할을 생성하고, 해당 IAM 역할을 Lambda 함수의 실행 역할로 설정한다. 버킷 정책에서 Lambda 함수의 실행 역할에도 액세스 권한을 부여하는지 확인한다.(Correct)**

****Explanation****

- AM 역할과 버킷이 다른 계정에 있는 경우, 여러분은 **IAM 역할과 버킷 정책 모두에 Amazon S3 권한을 부여해야 합니다.** 그러므로 이것이 제시된 활용 사례에서 AWS Lambda에 대한 액세스 권한을 부여하는 올바른 방법입니다.

q46

**AWS DataSync = Fsx, S3, EFS, (EBS는 안됨)**

q52

**다음 중 두 서비스의 요금 책정에 관한 설명으로 맞는 것은 무엇입니까?**

- EC2 시작 유형의 ECS는 EC2 인스턴스 및 EBS 볼륨 사용량에 따라 요금이 청구된다. Fargate 시작 유형은 컨테이너화된 애플리케이션이 요청하는 vCPU 및 메모리 리소스 양에 따라 요금이 부과된다.(Correct)
...
버퍼링 및 스로틀링에 사용할 수 있는 AWS 서비스 - Amazon Gateway, Amazon SQS, Amazon Kinesis
...
- **온디맨드 인스턴스와 스팟 인스턴스를 모두 사용해 원하는 규모, 성능, 비용을 달성하려면, 각 인스턴스 유형의 용량을 프로비저닝할 때 시작 템플릿(Launch Template)만을 사용해야 한다.(Correct)**
...
- **AWS Config 관리 규칙을 사용해 ACM으로 가져온 타사 SSL/TLS 인증서 중에 30일 이내에 만료됨으로 표시된 것이 있는지 확인한다. 30일 이내에 만료되는 인증서가 있을 경우 보안팀으로 SNS 알림을 보내도록 규칙을 구성한다.(Correct)**
    
    ![Untitled](UDemy%20AWS%202b59be6a38524858b63a4b507501541a/Untitled%20207.png)
    
- ACM으로 가져온 인증서의 `days to expiry` CloudWatch 지표를 모니터링한다. CloudWatch 경보를 생성하여 이러한 인증서의 `days to expiry`지표를 모니터링하고, 이를 기반으로 보안 팀에게 알림을 보내는 사용자 지정 액션을 실행한다.(Incorrect)
...
- **Amazon S3 스토리지 클래스 분석을 사용해 S3 Glacier 스토리지 클래스로 객체를 전환하여 스토리지 비용을 줄일 수 있는 권장 사항을 파악한다. 또한 수명 주기 정책을 사용하여 더 저렴한 스토리지 계층으로 객체를 자동 이동할 수도 있다.(Incorrect)**
- **AWS Cost Explorer의 리소스 최적화를 사용해 유휴 상태이거나 활용율이 낮은 EC2 인스턴스에 대한 보고서를 확인하고, AWS Compute Optimizer를 사용해 인스턴스 유형 권장 사항을 확인한다.(Correct)**

### **Explanation**

- 스토리지 클래스 분석은 ONEZONE_IA 또는 S3 Glacier 스토리지 클래스로의 이전에 관한 권고를 하지 않습니다.